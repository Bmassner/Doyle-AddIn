Module libVbCodeXport
    ...
    Public Function xprtModText() As Long
        ...
        d1 = dc(n1)
        send2clipBdWin10(n1 & vbCrLf & dumpKeyedText(d1, d1))
        Stop

        n2 = "C:\Users\athompson\Documents\dvl\libExt-rcvr-2017-0608.xlsm"
        d2 = dc(n2)
        send2clipBdWin10(n2 & vbCrLf & dumpKeyedText(d1, d2))
        Stop
    End Function
    ...
    Public Function lVCXg1f1(
    pj As VBIDE.VBProject
    ...
    Public Function lVCXg2f1(tx As String) As String
        ' Validate input string to avoid errors.
        If String.IsNullOrEmpty(tx) Then
            Return String.Empty ' Return an empty string if input is null or empty.
        End If

        ' Trim leading and trailing whitespace to standardize the input.
        tx = tx.Trim()

        ' Convert the input string to lowercase for case-insensitive comparison and processing.
        tx = tx.ToLower()

        ' Initialize a StringBuilder for efficient string concatenation.
        Dim result As New System.Text.StringBuilder()

        ' Iterate through each character in the processed input string.
        For i As Integer = 0 To tx.Length - 1
            ' Check if the character is a letter or digit.
            If Char.IsLetterOrDigit(tx(i)) Then
                ' Append the character to the result.
                result.Append(tx(i))
            End If
            ' Optionally, handle other characters or specific logic can be added here.
        Next
    ...
    Public Function lVCXg2f3(tx As String) As Scripting.Dictionary
        ...
        '     reserved by VB, but any unbroken  of non-space
    ...
    Public Function lVCXg4f1(dc As Scripting.Dictionary,
    Optional rf As Scripting.Dictionary = Nothing,
    Optional rt As Scripting.Dictionary = Nothing
        ...
        '     a sub of identically named procedures,
            ...
            With dc : For Each ky In .Keys
                    ...
                    With wk
                        If .Count > 0 Then
                            ...
                            ElseIf TypeOf ar(0) Is Scripting.Dictionary Then
                                'we have a sub of implementations
                                'keyed to module locations
                                'so need to go down a level
                                rt.Add(ky, lVCXg4f1(wk, rf,
                New Scripting.Dictionary))
    ...
    Public Function dxOfVbProcLocsInMod(
    cm As VBIDE.CodeModule
) As Scripting.Dictionary
        '
        ...
        rt = New Scripting.Dictionary
        ar = {
        {vbext_ProcKind.vbext_pk_Proc, ""},
        {vbext_ProcKind.vbext_pk_Get, ""},
        {vbext_ProcKind.vbext_pk_Let, "=#"},
        {vbext_ProcKind.vbext_pk_Set, "=@"}}
        ...
        With cm
            ...
            Do While dx < mx
                fw = dx
                Do While fw < mx
                    ck = .ProcOfLine(fw, vbext_ProcKind.vbext_pk_Proc)
                    If Len(ck) = 0 Then
                        fw = fw + 1
                    Else
                        tp = 0
                        ...
                        'tx = .Lines(dx, fw)
                        rt.Add(ck & ar(tp)(1), {dx, fw}) 'tx
                        ' REV[2023.05.05.1337]: replaced tx
                        ' .Lines with {dx, fw) to capture
    ...
    Public Function vbProcTextFromPrj(nm As String,
    Optional pj As VBIDE.VBProject = Nothing
) As String
        '
        ' vbProcTextFromPrj
    ...
    Public Function dxOfVbProcLocsInPrj(
    pj As VBIDE.VBProject
) As Scripting.Dictionary
        '
    ...
    cm As VBIDE.CodeModule
        ...
        '     , and Get Procedures. Let and  Procedures
    ...
    Public Function dcOfVbProcs_obs2023_0419(cm As VBIDE.CodeModule) As Scripting.Dictionary
        ...
        '     It does NOT look for Get, Let, or  Procedures.
        ...
        rt = New Scripting.Dictionary
        With cm
            If .Parent.Type = vbext_ComponentType.vbext_ct_StdModule Then
                mx = .CountOfLines
                dx = 1 + .CountOfDeclarationLines
                ...
                Do While dx < mx
                    ...
                    Do While fw < mx
                        ck = .ProcOfLine(fw, vbext_ProcKind.vbext_pk_Proc)
                        If Len(ck) > 0 Then
                            dx = .ProcStartLine(ck, vbext_ProcKind.vbext_pk_Proc)
                            fw = .ProcCountLines(ck, vbext_ProcKind.vbext_pk_Proc)
                            tx = .Lines(dx, fw)
                            rt.Add(ck, tx)
                            dx = dx + fw
                            fw = mx
    ...
    Public Function dcOfVbModules(
    vb As VBIDE.VBProject
) As Scripting.Dictionary
        Dim rt As Scripting.Dictionary
        Dim vc As VBIDE.VBComponent
        ...
        rt = New Scripting.Dictionary
        With vb
            If .Protection = vbext_ProjectProtection.vbext_pp_none Then
            ...
            Else
                ...
                Join({
                "' ",
                "' VB Project " & .Name & " Locked",
                "' Component Definitions and Data Unavailable",
                "' " _
            , vbCrLf})
            End If
        End With
    ...
    Public Function dcOfVbProcsFlat(
    pj As VBIDE.VBProject
) As Scripting.Dictionary
        '
    ...
    Public Function dcOfVbProjects(
    pjs As VBIDE.VBProjects
) As Scripting.Dictionary
        Dim rt As Scripting.Dictionary
    ...
    Public Function dcTxOfVbModule(
    dc As Scripting.Dictionary
) As Scripting.Dictionary
        Dim rt As Scripting.Dictionary
        Dim ky As Object
    ...
    Public Function dcTxOfVbProjMods(
    dc As Scripting.Dictionary
) As Scripting.Dictionary
        Dim rt As Scripting.Dictionary
        Dim ky As Object
    ...
    Public Function vbTextOfProcInDict(
    ...
    Public Function vbTextOfProcIn(
    nm As String, cm As VBIDE.CodeModule
) As String
        vbTextOfProcIn =
    vbTextOfProcInDict(
        nm, dcOfVbProcs(cm)
    ...
    Public Function vbTextOfProcInProject(
    ...
    Public Function send2clipBd(src As Object) As Object
        Dim ck As String
    ...
    Public Function getFromClipBd(Optional Fmt As Object = 1) As Object
    ...
    Public Function dumpKeyedText(
    d1 As Scripting.Dictionary,
    d2 As Scripting.Dictionary
) As String
        ...
        For Each ky In d1.Keys
            rt.Add("{" & ky & "}" & vbCrLf & d2(ky), 1)
        Next
        dumpKeyedText = Join(rt.Keys, vbCrLf)
    End Function
