...
Module modGPUpdateAT
        ...
        Else
            ...
            With invDoc
                ...
                Else
                    ...
                    '    qtRawMatl = System.Math.Round(prRmQty.Text, 4)
                    '    ' REV[2023.01.16.1605]
                    '    ' added System.Math.Round function to raw
                ...
                With .ComponentDefinition
                    ...
                    ElseIf InStr(1, "|D-HDWR|D-PTO|D-PTS|R-PTO|R-PTS|D-BAR|DSHEET|",
                    "|" & nmFamily & "|"
                ) > 0 Then
                ...
                If bomStruct = BOMStructureEnum.kNormalBOMStructure Then
                    ...
                    ' start of If block for bomStruct =BOMStructureEnum.kNormalBOMStructure
                        ...
                        With dcOb(dcRecSetDcDx4json(dcIn).Item(pnRawMaterial))
                            ...
                            If .Count > 0 Then 'Genius found something
                                ...
                                If .Count > 1 Then
                                    ...
                                    Debug.Print(pnModel & vbCrLf & vbTab & Join(.Keys, vbCrLf & vbTab)) 'prPartNum.Text
                    ...
                    If .SubType = guidSheetMetal Then 'for SheetMetal ---'
                        ...
                        Else 'we MIGHT, so lets get it
                            ...
                            If dcFP.Exists(pnThickness) Then
                                ...
                                ElseIf pnStock <> txTmp Then
                                    ...
                                    If ck = vbCancel Then
                                        ''Debug.Print(ConvertToJson(nuDcPopulator.Setting(pnModel, nuDcPopulator.Setting("from", prRawMatl.Text).Setting("into", pnStock).Dictionary).Dictionary, vbTab)
                                        Stop 'to check things out
                                        'send2clipBdWin10 ConvertToJson(nuDcPopulator.Setting(pnModel,nuDcPopulator.Setting("from",prRawMatl.Text).Setting("into",pnStock).Dictionary).Dictionary,vbTab)
                        ...
                        If ck = vbRetry Then
                            'Debug.Print(ConvertToJson({dcIn, dcFP, pnModel}, vbTab)) 'prPartNum.Text
                        ...
                        Else
                            ...
                            ElseIf Left$(pnStock, 2) = "LG" Then 'it's probably lagging
                                ...
                                Do
                                    ...
                                    "Verify Lagging " & pnStock & " for " & pnModel,
                                    pnStock
                                ))
                                    ...
                                    If Len(txTmp) > 0 Then
                                        ...
                                        End If
                                    Else
                                        'ck = MsgBox(Join({"Input appears to have been cleared.", "Are you sure you want to remove the", "current material," & pnStock & "?", "", "([Cancel] to debug)"}, vbCrLf), vbYesNoCancel, "Remove Material?")
                                        '"Select [No] to keep it.",
                                        '
                                        ck = MsgBox(
                                        Join({
                                            "No material entered.",
                                            "(perhaps entry was canceled?)",
                                        ...
                                        If ck = vbNo Then
                                            ck = MsgBox(
                                            "Do you want to keep " & pnStock & "?",
                                            vbYesNoCancel, "Keep Current?"
                                        )
                                            If ck = vbNo Then
                                ...
                                "vgMfiItems", "Family", "Item", pnStock
                                    ...
                                    Else
                                        ...
                                        ElseIf mtFamily = "D-BAR" Then
                                            ...
                                            & vbCrLf & "Otherwise, click [NO] to review." _
                                            & vbCrLf & "" _
                                            & vbCrLf & "( for debug, click [CANCEL] )"
                                        ) 'prRmQty.Text
                                                If ck = vbCancel Then
                                                    Stop 'to debug
                                                End If
                                            ...
                                            qtRawMatl = System.Math.Round(qtRawMatl, 4)
                                            If prRmQty.Text <> qtRawMatl Then
                                        ...
                                        Else
                                            ...
                                            Debug.Print(nu_FmGetList().AskUser(Join({"Unable to process", "the current Item.", "", "Copy the following ", "for later reference:", "", "Item: " & pnModel, "Matl: " & pnStock, "", vbCrLf})))
                            ...
                            ElseIf ck = vbYes Then
                                ...
                                ck = newFmTest2().AskAbout(invDoc,
                                    Join({
                                        ...
                                        , qtRawMatl & qtUnit _
                                        & " of " & pnStock _
                                        , "selected for Item ", "If this is okay, click [YES]" & vbCrLf & "(CANCEL to debug)"}, vbCrLf))
                                If ck = vbCancel Then
                                ElseIf ck = vbYes Then
                                    prRawMatl.Text = pnStock
                                    prRmQty.Text = qtRawMatl
                            ...
                            With CnGnsDoyle().Execute(sqlOf_simpleSelWhere(
                                ...
                                Else
                                    ...
                                    If mtFamily Like "?-MT*" Then
                                        ...
                                        'Debug.Print(pnModel & "[" & prRmQty.Text & qtUnit & "*" & pnStock & ": " & aiPropsDesign(pnDesc).Text & "]" ' prRawMatl.Text
                                        Debug.Print(pnModel & "[" _
                                            & qtRawMatl & qtUnit _
                                            & " of " & pnStock & ": " _
                                            & aiPropsDesign.item(pnDesc).Text _
                                    ...
                                    ElseIf mtFamily Like "?-PartDoc*" Then
                                        ...
                                        Else
                                            ...
                                            Else
                                                ...
                                                With NuDcPopulator(
                                                    ).Setting("D-RMT", "Doyle (typ. sheet metal)"
                                                    ...
                                                    nmFamily = userChoiceFromDc(
                                                        DcTransposed(.Dictionary()),
                                                        nmFamily
                                                    )
                                                End With
                                            End If
    ...
    Public Function dcAiDocComponents(AiDoc As Inventor.Document,
    Optional dc As Scripting.Dictionary = Nothing,
    ...
    Optional inclPhantom As Long = 0
        ...
        ElseIf AiDoc.DocumentType = DocumentTypeEnum.kAssemblyDocumentObject Then
            ...
            '
            rt = dcAssyDocComponents(
            AiDoc, rt, inclPhantom
        )
    ...
    Public Function dcAssyDocComponents(
    Assy As Inventor.AssemblyDocument,
    Optional dc As Scripting.Dictionary = Nothing,
    Optional inclPhantom As Long = 0
) As Scripting.Dictionary
        dcAssyDocComponents = dcAssyCompAndSub(
        Assy.ComponentDefinition.Occurrences,
        dc, inclPhantom
    )
    End Function

    Public Function dcAssyCompAndSub(
                ...
                With compOccFromProxy(invOcc) '(instead of just invOcc) [AT:2018.09.28]
                    ...
                    Else
                        ...
                        If .Visible And Not .Suppressed And Not .Excluded Then
                            ...
                            '  next statement (Ctrl-F9) at On Error above
                            ' and continue (F5)
                            ...
                            If Err.Number <> 0 Then
                                ...
                                Else
                                    Stop
                                End If
                                ' ocDef = .ContextDefinition 'NO!! This will NOT WORK!
                            End If
                            On Error GoTo 0
                        ...
                        Then
                                ...
                                Else 'assembly, check BOM Structure
                                    ...
                                    ElseIf .BOMStructure = BOMStructureEnum.kNormalBOMStructure Then 'we make it
                                        ...
                                        rt = dcAssyCompAndSub(.SubOccurrences,
                                    dcAddAiDoc(ocDef.Document, rt), inclPhantom
                                    ...
                                    ElseIf .BOMStructure = BOMStructureEnum.kInseparableBOMStructure Then 'maybe weldment?
                                        ...
                                        ElseIf tp = ObjectTypeEnum.kAssemblyComponentDefinitionObject Then
                                    ...
                                    ElseIf .BOMStructure = BOMStructureEnum.kPhantomBOMStructure Then '"phantom" component
                                        ...
                                        Else
                                            ...
                                            rt = dcAssyCompAndSub(.SubOccurrences, rt, inclPhantom)
                                        End If
                                    ElseIf .BOMStructure = BOMStructureEnum.kReferenceBOMStructure Then '"reference" component
                                    ...
                                    Else 'not sure what we've got
                                        Debug.Print(newFmTest2.AskAbout(ocDef.Document,
                                    "Unhandled Condition on this component.",
                                    "Going to Debug -- Click any button."
                                ))
                        ...
                        Else 'in case of improper skips
                            ...
                            '
                            ' USAGE:  breakpoint on following line when needed
    ...
    Public Function dcPropVals(
    dc As Scripting.Dictionary,
    Optional nonProp As Long = 0
) As Scripting.Dictionary
        '
    ...
    Public Function dcGeniusPropsAssy(
            ...
            Else
                With AiDoc
                    ...
                    With .ComponentDefinition
                        ...
                        ' and modified to collect existing value, if ,
                        ...
                        With .MassProperties
                            ...
                            If Err.Number Then
                                ...
                                , "(Mass will probably be incorrect)"
                            }, vbCrLf), vbOKCancel,
                                "ERROR(" & AiDoc.DisplayName & ")!"
                            )
                                If ck = vbCancel Then
                                    Stop
                                End If
    ...
    Public Function dcGeniusPropsPart(
    AiDoc As Inventor.PartDocument,
    Optional dc As Scripting.Dictionary = Nothing
) As Scripting.Dictionary
        If dc Is Nothing Then
            dcGeniusPropsPart =
        dcGeniusPropsPart(AiDoc,
        New Scripting.Dictionary)
        ...
        Else
            With invSheetMetalComp
                ...
                If Err.Number Then 'we've got a problem
                    If InStr(1,
                        aiDocument(.Document).FullFileName,
                        "\Doyle_Vault\Designs\purchased\"
                    ) Then
                        ...
                        '.BOMStructure = BOMStructureEnum.kPurchasedBOMStructure
                        'dc.Add("bomStruc", BOMStructureEnum.kPurchasedBOMStructure
                    Else
                    ...
                    If .HasFlatPattern Then
                        ...
                        With .FlatPattern
                            If .Body Is Nothing Then 'there's gonna be a problem
                                ...
                                With newFmTest2()
                                    If .AskAbout(
                                        invSheetMetalComp.Document,
                                        Join({
                                            "ISSUE WITH FLAT PATTERN:",
                                            "   NO BODY FOUND"
                                        }, vbCrLf),
                                        Join({
                                ...
                                dfHtThk = .Parameters.Item("Thickness").Text '/ cvLenIn2cm
                            ...
                            Else
                                ...
                                With nuAiBoxData().UsingInches(0
                                    ...
                                    dHeight = System.Math.Round(.SpanZ, 6) '(.MaxPoint.Z - .MinPoint.Z)

                                    ' Get the extent of the face.
                                    ' Extract the width, length and area from the range.
                                    dLength = System.Math.Round(.SpanX, 6) '(.MaxPoint.X - .MinPoint.X)
                                    dWidth = System.Math.Round(.SpanY, 6) '(.MaxPoint.Y - .MinPoint.Y)
                                End With
                                ...
                                With .Body.RangeBox
                                    ...
                                    ck = 0
                                    ck = ck + Math.Abs(.MaxPoint.Z - .MinPoint.Z - dHeight) '* cvLenIn2cm
                                    ck = ck + Math.Abs(.MaxPoint.X - .MinPoint.X - dLength) '* cvLenIn2cm
                                    ck = ck + Math.Abs(.MaxPoint.Y - .MinPoint.Y - dWidth) '* cvLenIn2cm
                                    ' UPDATE[2021.12.13]
                                    '     conversion operations removed
                                    '     since no longer converting
                                    '     before end stage
                                    If System.Math.Round(ck, 5) > 0 Then Stop 'BKPT-2021-1207-1158
                                ...
                                dfHtThk = System.Math.Round(dHeight - prThickness.Text, 6) '/ cvLenIn2cm
                                ' UPDATE[2021.12.13]
                                ...
                                dArea = dLength * dWidth 'System.Math.Round(, 6)
                        ...
                        With aiDocPart(.Document)
                            ...
                            '     '
                            ' NOTE[2021.11.09]
                            '     If UsingInches is  as shown above,
                            ...
                            With .UnitsOfMeasure
                                ...
                                If dfHtThk > 0.01 Then
                                    ...
                                    'Debug.Print(Join({aiDocument(.Document).FullFileName, _
                                    '    Format$(dHeight, "0.0000"), _
                                    '    Format$(prThickness.Text, "0.0000"), _
                                    '    Format$(dHeight - prThickness.Text, "0.0000") _
                                    '}, ":"))
                                    'Stop
                                Else
                                    strDVNs = ""
                ...
                With invSheetMetalComp
                    ...
                    Else 'For now, we're going to assume
                        ...
                        If Not .HasFlatPattern Then
                            ...
                            '
                            With newFmTest2().UseDict(.Document)
                                gn = .AskAbout(, , "NO FLAT PATTERN!" _
                                & vbCrLf & "Try to generate one?"
                            )
                            End With
                            If gn = vbYes Then
                        ...
                        If .HasFlatPattern Then
                            With .FlatPattern
                                ...
                                Else
                                    ...
                                    '     conversion operation removed since
                                    '     no longer converting before end stage
                                    dArea = System.Math.Round(dLength * dWidth, 6)
                            ...
                            If dfHtThk > 0 Then
                                With dcFlatPatSpansByVertices(.FlatPattern)
                                    ck = System.Math.Round(.Item("Z") - prThickness.Text, 6)
                                    If dfHtThk > ck Then
                                        dHeight = .Item("Z")
                                        dfHtThk = System.Math.Round(dHeight - prThickness.Text, 6)
                                        Debug.Print(System.Math.Round(.Item("X") - dLength, 6))
                                        Debug.Print(System.Math.Round(.Item("Y") - dWidth, 6))
                                        Debug.Print("") 'Breakpoint Landing
                                    End If
                                End With
                            End If
                        ...
                        With aiDocPart(.Document)
                            ...
                            With .UnitsOfMeasure
                                ...
                                If dfHtThk > 0.01 Then
                                    ...
                                    'Debug.Print(Join({
                                    '    aiDocument(.Document).FullFileName,
                                    '    Format$(dHeight, "0.0000"),
                                    '    Format$(prThickness.Text, "0.0000"),
                                    '    Format$(dHeight - prThickness.Text, "0.0000")
                                    '}, ":"))
                                    'Stop
                                Else
                                    strDVNs = ""
    ...
    Public Function dcWithProp(
        prop As Inventor.Property,
        Name As String, Value As Object,
        Optional rt As Scripting.Dictionary = Nothing
    ) As Scripting.Dictionary

        ' Initialize the dictionary if it's Nothing
        ...
        If prop Is Nothing Then
            Debug.Print("Error: Invalid InvProperty object.")
            Exit Function ' Or handle the error as appropriate
        End If

        ' Check if the Name is valid
        If Len(Trim(Name)) = 0 Then
            Debug.Print("Error: Property Name cannot be empty.")
            Exit Function ' Or handle the error as appropriate
        End If
    ...
    Public Function dcAddProp(
    aiProp As Inventor.Property,
    Optional dcIn As Scripting.Dictionary = Nothing
        ...
        If dcIn Is Nothing Then
            dcAddProp = dcAddProp(
            aiProp, New Scripting.Dictionary
        )
        Else
            dcOut = dcIn
    ...
    Public Function aiGetProp(
    aiProp As Inventor.Property,
    aiPropName As String,
    Optional Create As Long = 0
        ...
        'Debug.Print("PROP[" & aiProp.Name & "].ITEM[" & aiPropName & "]";

        ' Attempt to get an existing custom property named aiPropName.
        aiProp = aiProp.Item(aiPropName)
    ...
    Public Function aiProp(
        ByVal aiPropName As String,
        ByVal aiPropValue As Object
    ) As Inventor.Property

        ' Function to either retrieve an existing property or create a new one if it doesn't exist.
        ' Handles potential errors gracefully.
        Dim ThisApplication As Inventor.Application

        Try
            ' Attempt to get the property.  If it exists, this succeeds.
            aiProp = ThisApplication.ActiveDocument.PropertySets.Item("Inventor Summary Information").Item(aiPropName)
        Catch ex As Exception
            ' If the property doesn't exist, create it.
            Try
                aiProp = ThisApplication.ActiveDocument.PropertySets.Item("Inventor Summary Information").Add(aiPropValue, aiPropName)
            Catch ex2 As Exception
                ' Handle any errors during property creation.  Log the error, and return nothing.
                Debug.Print("Error creating property: " & aiPropName & " - " & ex2.Message)
                Return Nothing ' Or handle the error in a different way, such as throwing an exception.
            End Try
        End Try

        ' Set the property value.  This will update the property whether it was existing or newly created.
        Try
            aiProp.Value = aiPropValue
        Catch ex As Exception
            'Handle any errors during property setting
            Debug.Print("Error setting value for property: " & aiPropName & " - " & ex.Message)
            Return Nothing
        End Try
    ...
    Public Function ptNumShtMetal(
    aiSMdef As Inventor.SheetMetalComponentDefinition
) As String
                    ...
                    With rs
                        If .BOF And .EOF Then 'it wasn't found in Genius
                            ...
                            If Val(aiSMdef.ActiveSheetMetalStyle.Thickness) - invSheetMetalThickness < 0.0001 Then
                                ...
                                '     for now.
                                invGeniusMaterial = pnShtMetalHardCoded(
                        ...
                        Else 'Genius found it!
                            ...
                            With dc
                                If .Count > 0 Then
                                    invGeniusMaterial = .Keys(0) ' .Fields(0).Text
                                    If .Count > 1 Then
                                        invGeniusMaterial = userChoiceFromDc(
                                dc, invGeniusMaterial
                            ) 'nuSelFromDict '.Fields(0).Text
                                    End If
    ...
    Public Function sqlSheetMetal(
    Optional mtName As String = "",
    Optional thk As Double = 0
) As String
        ...
        '"Item Like '" & hdr2match & "%'"
        sqlSheetMetal = Trim$(Join({
        "SELECT Item, Description1",
        "FROM vgMfiItems",
        "WHERE " & IIf(Len(mtName) > 0,
            "Specification6 = '" & mtl2match & "'",
            "1=1"
        ),
        "  AND Family = " & IIf(hdr2match = "LG",
            "'D-PTS'", "'DSHEET'"
        ),
        IIf(hdr2match = "LG", "  AND Item LIKE 'LG%'",
            "  AND Specification1 = 'STANDARDSHEET'"
        ),
        IIf(thk > 0,
            "  AND Math.Abs(Thickness - " & thk2match & ") < 0.007",
            ""
        ),
        ";"
    }, vbCrLf))
    ...
    Public Function pnShtMetalHardCoded(
        invSheetMetalMaterial As String,
        invSheetMetalName As String
) As String
    ...
    Public Function dcAiDocPartNumbers(
    dcIn As Scripting.Dictionary
) As Scripting.Dictionary
        Dim rt As Scripting.Dictionary
        Dim ky As Object
