...
#End If
    ...
    WhereField As String, Matches As Object
        ...
        '   matching value
        If TypeOf Matches Is Array Then
...
with t as (
    select iType, bomStr
    from (values
        ('M', 51970), --BOMStructureEnum.kNormalBOMStructure
        ('R', 51973)  -- BOMStructureEnum.kPurchasedBOMStructure
    ) ls(iType, bomStr)
)
select
  i.Item [Part Number]
...
, dx as (
  select md.Item, PartDesc.Dim
  , PartDesc.Axis px
  , md.Axis mx
  from PartDesc inner join md
    --on PartDesc.Dim = md.Dim
    on PartDesc.Dim between md.dmn and md.dmx
)
--
--
select dx.Item
