Module app
    ...
    Public Sub Update_Genius_Properties()
        ...
        goAhead = MsgBox(
    Join({
        "Procedure Update_Genius_Properties is under MAJOR revision!",
        "Significant changes are in effect which may",
        "present issues in routine processing.",
        "Watch for any problems, and be prepared",
        "to respond appropriately."}), vbExclamation, "!!!!! WARNING !!!!!")

        ' Confirm User Request
        ' to process active Document
        goAhead = MsgBox(
    Join({
        "Are you sure you want to process this document?",
        "The process may require a few minutes depending on assembly size.",
        "Suppressed and excluded parts will not be processed."}), vbYesNo + vbQuestion, "Process Document Custom iProperties")
        If goAhead = vbYes Then
            ...
            If ActiveDoc.DocumentType = DocumentTypeEnum.kAssemblyDocumentObject Then
                ...
                goAhead = MsgBox(
                Join({
                    "Do you want to process the primary assembly?",
                    "If the main assembly document is just a collection",
                    "of separate parts and assemblies to be processed,",
                    "it's generally best not to include it in processing."}), vbYesNo + vbQuestion, "Include Main Assembly?")
            ...
            'ct = .Count
            'dx = 0
            'Set invProgressBar = InventorApp.CreateProgressBar( _
            '    ct, "Progressing: " _
            ')
            ...
            goAhead = MsgBox(
    Join({"Assembly Name:", ActiveDoc.DisplayName, "Process Completed ",
          "Copy report text", "(JSON format)", "to Clipboard? ",
          "(Cancel for Debug)"}, vbCrLf),
    vbYesNoCancel, "Update Complete")
    ...
    Public Sub Update_iPtAssy_Genius_Props()
        ...
        '   dc = gnsUpdtAll_iPart(aiDocPart(md).ComponentDefinition)
        'ElseIf TypeOf md Is Inventor.AssemblyDocument Then
        '   dc = gnsUpdtAll_iAssy(aiDocAssy(md).ComponentDefinition)
        'Else
    ...
    Private Function app() As String
        app = {"module app version date 2023.06.20", ""}(0)
    End Function
End Module